...
class image_converter:
    ...
    def callback(self,data):
        ...
        img = cv2.imread('/home/user0001/swab1.0.0_ws/src/swab-ros-main/camcam/src_pic/oral.png')
        ...
        if state == 1:
            ...
            if record_1 == 1:
                cv2.rectangle(resized_cropped,(80,0),(600,40),(255,255,255),-1)
                cv2.rectangle(resized_cropped,(80,0),(600,40),info_color,4)
                cv2.putText(resized_cropped, "double click left button to confirm.", (200,18), cv2.FONT_HERSHEY_PLAIN,1, info_color, 1, cv2.LINE_AA)
                cv2.putText(resized_cropped, "or click left button to reselect.", (200,30), cv2.FONT_HERSHEY_PLAIN,1, info_color, 1, cv2.LINE_AA)
                draw_x1 = 0
                draw_y1 = 0
                draw_x2= 0
                draw_y2 = 0
            ...
            if record_1 ==2:
                cv2.rectangle(resized_cropped,(80,0),(600,40),(255,255,255),-1)
                cv2.rectangle(resized_cropped,(80,0),(600,40),info_color,4)
                cv2.putText(resized_cropped, "double click right button when finish.", (200,25), cv2.FONT_HERSHEY_PLAIN,1, info_color, 1, cv2.LINE_AA)
        if state == 2:
            cv2.line(resized_cropped, (target_x-15,0), (target_x-15,height_re), color_grid, thickness=1)
            cv2.line(resized_cropped, (target_x+15,0), (target_x+15,height_re), color_grid, thickness=1)
            cv2.line(resized_cropped, (0,target_y-15), (width_re,target_y-15), color_grid, thickness=1)
        ...
        if state == 3:
            ...
            if swab_state==0:                
                cv2.putText(resized_cropped, swabing_info, (230,18), cv2.FONT_HERSHEY_PLAIN,1, info_color, 1, cv2.LINE_AA)
                cv2.putText(resized_cropped, swabing_info2, (120,32), cv2.FONT_HERSHEY_PLAIN,1, info_color, 1, cv2.LINE_AA)
            if swab_button_ui == 1:
        ...
        if state == 3 and swab_state == 0 :
            state_3_ban = 0
        if state == 3 and swab_button_ui == 3:
            cv2.rectangle(resized_cropped,(80,0),(600,40),(255,255,255),-1)
            cv2.rectangle(resized_cropped,(80,0),(600,40),info_color,4)
            cv2.putText(resized_cropped, "double click right button when finish.", (200,18), cv2.FONT_HERSHEY_PLAIN,1, info_color, 1, cv2.LINE_AA)
...
def dc(event,x,y,flags,param):
    ...
    if state==1 and state_1_ban==0:
        ...
        if event == cv2.EVENT_LBUTTONDOWN:
            ...
            mouseX = x
            mouseY = y
        if event == cv2.EVENT_LBUTTONDBLCLK and record_1 == 1:
            shot_1 = 1
            mouseX = x
            mouseY = y
    ...
    if state==2 and state_2_ban==0:
        ...
        if event == cv2.EVENT_LBUTTONDOWN:
            ...
            mouseX = x
            mouseY = y
        if event == cv2.EVENT_LBUTTONDBLCLK and record_2 == 1:
            shot_2 = 1
            mouseX = x
            mouseY = y
        if event == cv2.EVENT_MBUTTONDBLCLK:
